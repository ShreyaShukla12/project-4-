		...
		<center>
			...
			<%
                    ...
                    Iterator<CourseBean> it = list.iterator();
	       			...
	       			/* int sno = Integer.parseInt((sre == null || sre == "") ? "0" : sre);
	       				List<CourseBean> list = new ArrayList();
						if (index > 0) {
							
							Iterator it1 = listA.iterator();
							while (it1.hasNext()) {
								bean = (CourseBean) it1.next();
								if (sno) {
									list.add(bean);  */
								/* }else{
				...
				<tr>
					<td align="center"><label> Course Name :</label> <%=HTMLUtility.getList("cname", String.valueOf(bean.getId()), courselist) %>
					 ...
					 --%>&emsp; <label> Duration :</label> <input type="text"
						name="duration" placeholder="Enter Duration of Course" size="20"
						value="<%=DataUtility.getStringData(bean.getDuration())%>">
						&emsp; <%--  <label>Discription</label>
					 <input type="text" name ="Discription" placeholder="Enter Discription of college" size="20"
					 value="<%=DataUtility.getStringData(bean.getDiscription())%>"> --%>
						<%-- <label>S.No</label> <input type="text" name="S.no"
						placeholder="Enter by S.No" value="<%=Sno%>">  --%>
			...
			<table width="100%">
				<tr>
					...
					<td><input type="submit" name="operation" disabled="disabled"
						value="<%=CourseListCtl.OP_PREVIOUS%>"> <%}else{ %>
